...
module.exports = {
    ...
    'unpack: unknown format instruction': function() {
        var error; try { error = false; unpack('Z', toString[0x32]); } catch (e) { error = e; } Assert.ok(error !== false);
    },
    'unpack: input too short': function() {
        Assert.throws(function(){unpack('NN', toString([0x65]));});
        Assert.throws(function(){unpack('c', "");});
        Assert.throws(function(){unpack('n*', toString([0x65]));});
        Assert.throws(function(){unpack('n', toString([0x65]));});
    }
};
